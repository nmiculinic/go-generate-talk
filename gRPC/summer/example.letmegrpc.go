...
func Serve(httpAddr, grpcAddr string, stringer func(req, resp interface{}) ([]byte, error), opts ...grpc.DialOption) {
...
func NewHandler(grpcAddr string, stringer func(req, resp interface{}) ([]byte, error), opts ...grpc.DialOption) (http.Handler, error) {
	conn, err := grpc.Dial(grpcAddr, opts...)
	if err != nil {
		return nil, err
	}
	mux := http.NewServeMux()
	MathClient := NewMathClient(conn)
	MathServer := NewHTMLMathServer(MathClient, stringer)
	mux.HandleFunc("/Math/Sum", MathServer.Sum)
	return mux, nil
}

type htmlMath struct {
	client   MathClient
	stringer func(req, resp interface{}) ([]byte, error)
}

func NewHTMLMathServer(client MathClient, stringer func(req, resp interface{}) ([]byte, error)) *htmlMath {
	return &htmlMath{client, stringer}
}

var FormMath_Sum string = `<div class="container"><div class="jumbotron">
	<h3>Math: Sum</h3>
...
var nodeFactory = {"SumRequest_RootKeyword": buildSumRequest_RootKeyword(emptyIfNull(null)),}
	function buildSumRequest_RootKeyword(json) {
if (json == undefined) {
		return "";
	}
	
var s = '<div class="node" type="SumRequest_RootKeyword" fieldname="RootKeyword" repeated="false">';
s += '<div class="fields" fieldname="a">';
				var a = getList(json, "a");
				for (var i = 0; i < a.length; i++) {
					s += '<div class="field form-group"><label class="col-sm-2 control-label">a: </label><div class="col-sm-8"><input class="form-control" name="a" type="number" step="any" repeated="true" '+setRepValue(json["a"][i])+'/></div><div class="col-sm-2"><a href="#" class="del-field btn btn-warning btn-sm" role="button">Remove</a></div></div>';
				}
				s += '</div>';
				s += '<a href="#" fieldname="a" class="add-elem btn btn-info btn-sm" role="button" type="number">add a</a>';
	...
	var root = $(nodeFactory["SumRequest_RootKeyword"]);
	...
	$("#form > .children").html(buildSumRequest_RootKeyword(json));
	activateLinks(root);
	$("a[id=submit]").click(function(ev) {
		ev.preventDefault();
		c = getChildren($("#form"));
		j = JSON.stringify(c["RootKeyword"]);
		window.location.assign("./Sum?json="+j);
	});
}
...
func (this *htmlMath) Sum(w http.ResponseWriter, req *http.Request) {
	w.Write([]byte(Header(`Math`, `Sum`)))
	...
	w.Write([]byte(FormMath_Sum))
	if someValue {
		reply, err := this.client.Sum(context.Background(), msg)
		if err != nil {
			if err != io.EOF {
				w.Write([]byte("<div class=\"alert alert-danger\" role=\"alert\">" + err.Error() + "</div>"))
				return
